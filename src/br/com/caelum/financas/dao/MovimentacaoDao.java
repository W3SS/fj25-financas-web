...
public class MovimentacaoDao {
	...
	public void adiciona(Movimentacao movimentacao) {
		//AGORA QUE O CDI INJETA O MANAGE PARA MEU DAO, PRECISO CRIAR UM JOIN TRANSACTION
	...
	public List<Movimentacao> listaTodasMovimentacoes(Conta conta){
		...
		return query.getResultList();//DA WARNING PORQUE NAO TEM CERTEZA DO TIPO DE RETORNO, NA LINHA DE BAIXO VOU USAR TYPED QUERY*/
	...
	public List<Movimentacao> listaPorValoreTipo(BigDecimal valor, TipoMovimentacao tipo){
		...
		return query.getResultList();ASSIM DA WARN*/
		
		TypedQuery<Movimentacao> query = this.manager.createQuery(jpql, Movimentacao.class);
		query.setParameter("valor", valor);
		query.setParameter("tipo", tipo);
		
		//IMPLEMENTANDO O CACHE DE QUERY
	...
	public List<ValorPorMesEAno>listaMesesComMovimentacoes(Conta c, TipoMovimentacao tipo){
		String jpql = "select new br.com.caelum.financas.dto.ValorPorMesEAno(month(m.data), year(m.data), sum(m.valor)) from Movimentacao m where m.conta= :conta and m.tipoMovimentacao= :tipo group by year(m.data), month(m.data)";
		Query query = this.manager.createQuery(jpql);
		query.setParameter("conta", c);
		query.setParameter("tipo", tipo);
		return query.getResultList();//WARN POIS NAO DA CERTEZA DO RETORNO TIPADO PARA VALORPORMESEANO
	}	
